...
class ProfileTest {
    ...
    void 프로필을_생성한다() {
        // given & when & then
        Member member = new Member("fancy@gmail.com", "fancy", "fancy_profile.png", SocialType.GOOGLE);
        assertDoesNotThrow(()-> new Profile(member, 팬시_닉네임, 팬시_나이, 팬시_성별, 팬시_성격들, 팬시_자기소개
                , 팬시_메인_이미지, 팬시_서브_이미지, 팬시_직업, 팬시_도시, 팬시_도시_구, 팬시_직업_선택여부, 팬시_서브_이미지_선택여부,팬시_주소_선택여부 ));
    }
    @DisplayName("닉네임이 비거나 20글자 넘으면 예외를 던진다.")
    @ParameterizedTest
    @ValueSource(strings = {"", "일이삼사오육칠팔구십" + "일이삼사오육칠팔구십일", "하나둘셋넷다섯여섯일곱여덟아홉열" +"하나둘셋넷다섯여섯일곱여덟아홉열" + "하나둘셋넷다섯여섯일곱여덟아홉열"})
    void 닉네임이_20글자_넘으면_예외를_던진다(final String nickname) {
        // given & when & then
        Member member = new Member("fancy@gmail.com", "fancy", "fancy_profile.png", SocialType.GOOGLE);
